...
body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Helvetica,Arial,sans-serif;background:var(--bg);color:var(--text);-webkit-font-smoothing:antialiased;font-size:14px}
...
.modal{background:#fff;border-radius:18px;box-shadow:var(--shadow-lg);width:100%;max-width:540px;animation:pop .16s ease}
...
.cbox h4{margin:0 0 14px;font-size:12px;font-weight:800;letter-spacing:.7px;text-transform:uppercase;color:var(--faint);display:flex;align-items:center;gap:8px}
  ...
  <div class="main">
    <div class="topbar">
      <div class="gsearch">
        ...
        <input id="gq" placeholder="Search orders, customer name, phone number..." autocomplete="off"/>
      </div>
...
function openModal(inner,fbuttons,cb,title){
  ...
  box.dataset.cb=cb?'1':'0';
  box._cb=cb;
  box._btn=btns;
  bindModalBtns();
}
function openModalFull(inner,title,cb){
  var box=document.getElementById('modalBox');
  box.innerHTML='<div class="m-head"><h3>'+esc(title||'')+'</h3><button class="m-x" data-mb="x">×</button></div><div class="m-body">'+inner+'</div>';
  box._cb=cb||null;box._btn='';
  document.getElementById('modalOv').classList.add('open');
  bindModalBtns();
}
function setModalTitle(t){var h=document.querySelector('#modalBox .m-head h3');if(h)h.textContent=t;}
function closeModal(){document.getElementById('modalOv').classList.remove('open');}
function bindModalBtns(){
  document.querySelectorAll('[data-mb]').forEach(function(b){
    b.onclick=function(){
      var id=b.getAttribute('data-mb');
      if(id==='x'){closeModal();return;}
      var box=document.getElementById('modalBox');
      if(box._cb){box._cb(id);}
      else{var fn=handler(id,b.getAttribute('data-arg'));if(fn)fn();}
      if(box._close!==false)closeModal();
    };
  });
}
function handler(id,arg){
  var map={
    'close':closeModal,
    'reload':function(){route();}
  };
  return map[id]||function(){};
}
...
function vDashboard(){
  ...
  Promise.all([
    ...
    var alerts=inv.filter(function(i){return i.status==='out'||i.status==='low';}).slice(0,8);
    ...
    var recRows=recent.map(function(o){
      return '<tr class="trow" data-go="#/orders/'+esc(o.id)+'">'+
    ...
    page.innerHTML=
      '<div class="p-head"><div class="p-title"><h1>Dashboard</h1><p class="h-intro">An overview of your store right now.</p></div>'+
...
function renderOrderDetail(){
  ...
  var pageHtml=
    ...
    '</div></div>'+
    '<div class="grid2">'+
      '<div class="cbox"><h4>'+IC.orders+' Current Status</h4>'+
      ...
      '</div>'+
      '<div>'+
        '<div class="cbox" style="margin-bottom:14px"><h4>'+IC.customers+' Customer Details</h4>'+
        ...
        '</div>'+
        '<div class="cbox"><h4>'+IC.pin+' Delivery Address</h4>'+
    ...
    '<div class="grid2" style="margin-top:14px">'+
      '<div class="cbox"><h4>'+IC.products+' Order Items</h4>'+
      ...
      '</div>'+
      '<div>'+
        '<div class="cbox"><h4>'+IC.reports+' Summary</h4>'+
    ...
    '<div class="cbox" style="margin-top:14px"><h4>'+IC.delivery+' Order Actions</h4>'+
      '<div style="display:flex;gap:10px;flex-wrap:wrap">'+nextBtn+paidBtn+
        '<button class="btn" data-act="copy" data-arg="'+esc(o.id)+'">'+IC.clipboard+'&nbsp;Copy Order ID</button>'+
        (NEXT[o.status]?'<button class="btn danger" data-act="cancel" data-arg="'+esc(o.id)+'">Cancel Order</button>':'')+
      '</div></div>';
...
function openProductForm(pid){
  ...
  openModal(
    '<div class="f-group"><label>Product Name</label><input id="pf-name" value="'+(p?esc(p.item):'')+'"/></div>'+
    '<div class="f-group"><label>Image URL (optional)</label><input id="pf-img" value="'+(p?esc(p.image):'')+'" placeholder="https://..."/><div class="t-sub" style="margin-top:4px">Leave empty to use the ZIPRA placeholder image.</div></div>'+
    '<div class="f-row">'+
      '<div class="f-group"><label>Category</label><select id="pf-cat">'+cats+'<option value="">+ New category</option></select></div>'+
      '<div class="f-group"><label>New Category Name</label><input id="pf-catnew" placeholder="Only if creating new"/></div>'+
    '</div>'+
    '<div class="f-row3">'+
      '<div class="f-group"><label>Price (₹)</label><input id="pf-price" type="number" step="0.01" value="'+(p?p.price:'')+'"/></div>'+
      ...
      '<div class="f-group"><label>GST %</label><input id="pf-gst" type="number" step="0.1" value="'+(p?p.gst_rate:0)+'"/></div>'+
    '</div>'+
    '<div class="f-row">'+
      '<div class="f-group"><label>Unit</label><input id="pf-unit" value="'+(p?esc(p.unit):'kg')+'"/></div>'+
      '<div class="f-group"><label>Stock</label><input id="pf-stock" type="number" value="'+(p?p.stock:0)+'"/></div>'+
    '</div>'+
    '<div class="f-row">'+
      '<div class="f-group"><label>Low Stock Level</label><input id="pf-low" type="number" value="'+(p?p.low_stock_level:5)+'"/></div>'+
      '<div class="f-group"><label>Status</label><select id="pf-active"><option value="1"'+(p&&p.active?' selected':'')+'>Active</option><option value="0"'+(p&&!p.active?' selected':'')+'>Disabled</option></select></div>'+
    '</div>',
    [{id:'p-save',title:isNew?'Add Product':'Save Changes',cls:'primary'}],
    function(id){
      if(id!=='p-save')return;
      ...
      req.then(function(){toast(isNew?'Product added':'Product saved');vProducts();}).catch(function(){toast('Save failed','err');});
    },
...
function openAdjust(pid,op){
  ...
  openModal(
    ...
    '<div style="margin-bottom:14px"><b>'+esc(i.name)+'</b><div class="t-sub">Current stock: <b>'+i.stock+' '+esc(i.unit)+'</b></div></div>'+
    '<div class="f-row">'+
      '<div class="f-group"><label>Operation</label><select id="adj-op"><option value="+"'+(op!=='-'?' selected':'')+'>Add</option><option value="-"'+((op==='-'||op==='')?' selected':'')+'>Remove</option></select></div>'+
      '<div class="f-group"><label>Quantity</label><input id="adj-qty" type="number" value="1"/></div>'+
    '</div>'+
    '<div class="f-group"><label>Reason</label><input id="adj-reason" placeholder="e.g. new stock received, damaged, count"/></div>',
    [{id:'adj-go',title:'Apply',cls:'primary'}],
    function(id){
      if(id!=='adj-go')return;
      ...
      api('/api/inventory/adjust',{method:'POST',body:{productId:pid,delta:delta,reason:document.getElementById('adj-reason').value.trim()||'Manual adjustment'}})
        .then(function(){toast('Stock updated');vInventory();}).catch(function(e){toast((e.message||'Adjust failed'),'err');});
    },
    'Adjust Stock'
  );
}
...
function vCustomer(){
  api('/api/customers/'+state.arg).then(function(d){
    ...
    var rows=(c.orders||[]).map(function(o){
      return '<tr class="trow" data-go="#/orders/'+esc(o.id)+'"><td class="t-id">'+esc(o.id)+'</td><td class="t-sub">'+fmtTS(o.createdAt)+'</td><td style="text-align:right" class="money">'+money(o.total)+'</td><td>'+pill(o.status)+'</td></tr>';
    }).join('');
    var page=document.getElementById('page');
    page.innerHTML=
      ...
      '<div style="padding:14px 18px;border-top:1px solid var(--line);display:flex;gap:9px">'+
        '<a class="btn" href="https://wa.me/'+esc(String(c.phone).replace(/^\\+/,''))+'" target="_blank" rel="noopener">'+IC.phone+'&nbsp;WhatsApp</a>'+
        '<button class="btn" data-go="#/orders">'+IC.orders+'&nbsp;View Orders</button>'+
      '</div></div>';
    bindDelegates();
...
function openPartnerForm(id){
  ...
  openModal(
    ...
    '<div class="f-group"><label>Status</label><select id="pn-on"><option value="1"'+(p&&p.online?' selected':'')+'>Online</option><option value="0"'+(p&&!p.online?' selected':'')+'>Offline</option></select></div>',
    [{id:'pns',title:id?'Save':'Add',cls:'primary'}],
    function(aid){
      if(aid!=='pns')return;
      ...
      req.then(function(){toast(id?'Partner updated':'Partner added');vPartners();}).catch(function(){toast('Save failed','err');});
    },
...
function openPromoForm(id){
  ...
  openModal(
    ...
    '</div>'+
    '<div class="f-row">'+
      '<div class="f-group"><label>Type</label><select id="pr-type"><option value="percent"'+(p&&p.type==='percent'?' selected':'')+'>Percent</option><option value="fixed"'+(p&&p.type==='fixed'?' selected':'')+'>Fixed amount</option></select></div>'+
      '<div class="f-group"><label>Value</label><input id="pr-value" type="number" step="0.01" value="'+(p?p.value:10)+'"/></div>'+
    '</div>'+
    '<div class="f-row">'+
      '<div class="f-group"><label>Min Order (₹)</label><input id="pr-min" type="number" value="'+(p?p.minOrder:0)+'"/></div>'+
      '<div class="f-group"><label>Status</label><select id="pr-active"><option value="1"'+(p&&p.active?' selected':'')+'>Active</option><option value="0"'+(p&&!p.active?' selected':'')+'>Inactive</option></select></div>'+
    '</div>',
    [{id:'prs',title:id?'Save':'Create',cls:'primary'}],
    function(aid){
      if(aid!=='prs')return;
      ...
      req.then(function(){toast(id?'Promotion saved':'Promotion created');vPromotions();}).catch(function(){toast('Save failed','err');});
    },
...
function openAddOrder(){
  api('/api/products').then(function(d){
    ...
    openModal(
      '<div class="f-row">'+
        '<div class="f-group"><label>Customer Name</label><input id="ao-name"/></div>'+
        '<div class="f-group"><label>Phone (WhatsApp)</label><input id="ao-phone" placeholder="91XXXXXXXXXX"/></div>'+
      '</div>'+
      ...
      '<div class="f-row">'+
        '<div class="f-group"><label>Payment Method</label><select id="ao-pm"><option value="cod">Cash on Delivery</option><option value="online">Online Payment</option></select></div>'+
        '<div class="f-group"><label>Order Status</label><select id="ao-st"><option value="received">Received</option><option value="confirmed">Confirmed</option><option value="preparing">Preparing</option><option value="out_for_delivery">Out for Delivery</option><option value="delivered">Delivered</option></select></div>'+
      '</div>'+
      '<div id="ao-sum" class="t-sub" style="margin-top:2px">Subtotal ₹0 · Delivery '+money(fee)+' · Total '+money(fee)+'</div>',
      [{id:'ao-go',title:'Create Order',cls:'primary'}],
      function(id){
        if(id!=='ao-go')return;
...
function openEditOrder(idc){
  api('/api/orders/'+idc).then(function(d){
    ...
    openModal(
      '<div class="f-row">'+
        '<div class="f-group"><label>Customer Name</label><input id="eo-name" value="'+esc(o.name||'')+'"/></div>'+
        '<div class="f-group"><label>Phone</label><input id="eo-phone" value="'+esc(o.waId||'')+'"/></div>'+
      '</div>'+
      ...
      '</div></div>'+
      '<div class="f-row">'+
        '<div class="f-group"><label>Delivery Fee (₹)</label><input id="eo-fee" type="number" value="'+o.deliveryFee+'"/></div>'+
        '<div class="f-group"><label>Payment Method</label><select id="eo-pm"><option value="cod"'+(o.paymentMethod!=='online'?' selected':'')+'>COD</option><option value="online"'+(o.paymentMethod==='online'?' selected':'')+'>Online</option></select></div>'+
      '</div>',
      [{id:'eo-save',title:'Save Changes',cls:'primary'}],
      function(id){
  ...
  switch(act){
    ...
    case 'next':
      conf('Advance order '+arg+' to "'+LABEL[ctx.st]+'"?').then(function(yes){
        ...
        api('/api/orders/'+arg+'/status',{method:'POST',body:{status:ctx.st,notify:true}})
          .then(function(d){toast((d.notified?'Status updated & customer notified':'Status updated'));if(state.view==='orders'&&state.arg)vOrderDetail();else route();})
          .catch(function(e){toast((e.message||'Update failed'),'err');});
      });
      break;
    case 'pay':
      api('/api/orders/'+arg+'/payment',{method:'POST',body:{status:'Paid'}})
        .then(function(){toast('Marked as Paid');if(state.view==='orders'&&state.arg)vOrderDetail();else route();})
        .catch(function(e){toast((e.message||'Update failed'),'err');});
      break;
    case 'cancel':
      conf('Cancel order '+arg+'? Stock will be restored.').then(function(yes){
        ...
        api('/api/orders/'+arg+'/status',{method:'POST',body:{status:'cancelled',notify:true}})
          .then(function(){toast('Order cancelled');if(state.view==='orders'&&state.arg)vOrderDetail();else route();})
    ...
    case 'contact':
      api('/api/orders/'+arg).then(function(d){
        var o=d.order;
        openModal(
          '<div style="margin-bottom:10px">Send a WhatsApp message to <b>'+esc(o.name||'')+'</b> at <b>'+esc(o.waId||'')+'</b></div>'+
          ...
          [{id:'ct-go',title:'Send Message',cls:'primary'}],
          function(id){
            if(id!=='ct-go')return;
            api('/api/orders/'+arg+'/notify',{method:'POST',body:{message:document.getElementById('ct-msg').value}})
              .then(function(d){toast(d.sent?'Message sent to customer':'Could not send',d.sent?'':'err');}).catch(function(e){toast((e.message||'Send failed'),'err');});
          },
          'Contact Customer'
        );
      });
    ...
    case 'assign':
      api('/api/delivery-partners').then(function(d){
        ...
        openModal(
          ...
          [{id:'ap-go',title:'Assign',cls:'primary'}],
          function(id){
            if(id!=='ap-go')return;
            ...
            api('/api/orders/'+arg+'/assign',{method:'POST',body:{partnerId:Number(sel.value)}})
              .then(function(){toast('Delivery partner assigned');vOrderDetail();}).catch(function(e){toast((e.message||'Assign failed'),'err');});
          },
...
function toggleNotif(){
  ...
  api('/api/orders?status=received').then(function(data){
    ...
    var rows=items.map(function(o){
      var c=COLORS[o.status]||COLORS.received;
      return '<div class="dd-item" data-go="#/orders/'+esc(o.id)+'"><span class="notif-ic" style="background:'+c.bg+';color:'+c.fg+'">'+IC.check+'</span><div><div class="d-t">'+esc(o.id)+' · '+LABEL[o.status]+'</div><div class="d-s">'+esc(o.name||'')+' · '+money(o.total)+' · '+fmtTS(o.createdAt)+'</div></div></div>';
    }).join('');
...
document.addEventListener('click',function(e){
  ...
  if(tr){e.preventDefault();go(tr.getAttribute('data-go'));}
